...
export default function VenuesPage() {
  ...
  useEffect(() => {
    ...
    setBookingForm((f) => ({
      ...f,
      start_time: s.toISOString().slice(0, 16),
      end_time: e.toISOString().slice(0, 16),
    }));
    load();
  }, []);
  ...
  return (
    <div>
      ...
      <div className="panel">
        ...
        <form onSubmit={createVenue}>
          ...
          <select value={venueForm.venue_type} onChange={(e) => setVenueForm({ ...venueForm, venue_type: e.target.value })}>
            <option>实验室</option><option>会议室</option><option>讨论室</option><option>自习室</option><option>活动室</option><option>体育场地</option>
          </select>
          ...
          <select value={venueForm.status} onChange={(e) => setVenueForm({ ...venueForm, status: e.target.value })}>
            <option value="available">可预约</option><option value="maintenance">维护中</option><option value="unavailable">停用</option>
          </select>
      ...
      <div className="panel">
        ...
        {venues.length === 0 ? <p>暂无可预约场地，请先新增场地。</p> : null}
      ...
      <table className="table">
        <thead>
          <tr>
            <th>活动名称</th><th>场地名称</th><th>申请人</th><th>状态</th><th>开始时间</th><th>结束时间</th><th>操作</th>
          </tr>
        </thead>
        <tbody>
          {bookings.map((b) => (
            <tr key={b.id}>
              ...
              <td>
                ...
                <button onClick={() => act(() => venuesApi.reject(b.id, "不符合条件"), "预约已拒绝")} className="btn-secondary">拒绝</button>
                <button onClick={() => act(() => venuesApi.cancel(b.id, "用户取消"), "预约已取消")} className="btn-danger">取消</button>
              </td>
            </tr>
          ))}
          {bookings.length === 0 ? <tr><td colSpan={7}>暂无数据，请先创建预约申请。</td></tr> : null}
        </tbody>
      </table>
    </div>
  );
